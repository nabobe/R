#Anzahle Graphiken
png(filename="figure.png",width=180,height=120,units="mm",res=900,pointsize=12)
par(mfrow=c(1,3))
...
mtext(side=2,line=2.5,"weber fractions", cex=1)
mtext(side=1,line=1,"visual        auditiv", cex=1)
abline(h=0)

x.text<-colMeans(mp)
text(c("***"),x=x.text,y=0.81, cex=2)

mtext(side=3,line=2,expression(paste(bold("(A)  "),"Subsecond range")),cex=1,at=0.52)
abline(h=0)
...
mtext(side=3,line=2,expression(paste(bold("(B)  "),"Second range")),cex=1,at=0.52)
abline(h=0)
...
mtext(side=3,line=0.4,expression(paste(bold("(C)  \n"),"Second range \nadjusted means")),cex=1,at=0.52)
abline(h=0)
axis(2,pos=0.15,labels=FALSE,at=seq(0,.85, by=.20))
